...
public class AIMailExchangeSimulator {
	class Tools {
		 ...
		 * WebDriverWait(driver, Duration.ofSeconds(10)); driver.get(url); wait.until(
		 * (ExpectedCondition<Boolean>) wd -> ((JavascriptExecutor)wd)
		 * .executeScript("return document.readyState") .equals("complete")); var src =
		 * driver.getPageSource(); driver.close(); boolean isMd = false; try { var doc =
		 * Parser.htmlParser().parseInput(src,(new URI(url)).getHost()); for(var e :
		 * doc.select("a[href]")) { e.attr("href", e.absUrl("href"));
		 * e.text(e.text().trim()); } var converter = new CopyDown(); src =
		 * converter.convert(doc.toString()); isMd = true; } catch (URISyntaxException
		 * e) { // TODO Auto-generated catch block e.printStackTrace(); } var fn =
		 * "./archive/fetched_url_" + getTimeStamp(new Date()) + "_" +
		 * url.replaceAll(":","").replaceAll("[/%&?]", "_") + (isMd ? ".md" : "");
		 * PrintWriter pw; try { pw = new PrintWriter(fn); pw.print(src); pw.flush();
		 * pw.close(); MainService.ingestSingleFile(fn); } catch (FileNotFoundException
		 * e) { ; } return src; }
		 */
	...
	public static void saveEmail(Date date, String mail) throws FileNotFoundException {
	...
	public static String generateMIMEEmail(Date now, String subject, String senderName, String senderEmail,
			List<List<String>> destinations, String content) {
		return generateMIMEEmail(now, subject, senderName, senderEmail, destinations, content, null);
	}

	public static String generateMIMEEmail(Date now, String subject, String senderName, String senderEmail,
			List<List<String>> destinations, String content, String asAReplyTo) {
		...
		mimeEmail.append("To: ")
				.append(Joiner.on("; ").join(destinations.stream().map(p -> p.get(0) + " <" + p.get(1) + ">").toList())
						+ "\r\n");
		...
		if (asAReplyTo != null) {
			...
			var s = asAReplyTo;
			if (s.startsWith("MIME-Version: 1.0"))
				s = s.replaceFirst("MIME-Version: 1.0", "").trim();
	...
	private List<String> getUserPersona() {
		return getPersonaFromName(comboUserPersona.getSelectedItem().toString());
	}

	private void interact(String str, String subject, String lastEmail, List<String> userPersona,
			List<List<String>> agentPersona) {
	...
	private void interact(String str, String subject, String lastEmail, List<String> userPersona,
			List<List<String>> agentPersonas, boolean thenSwapped) {
		new Thread(() -> {
			endisable(false);
			interactLock++;
			var now = new Date();

			var mail = ((str == null) ? lastEmail
					: generateMIMEEmail(now, subject, userPersona.get(0), userPersona.get(1), agentPersonas, str,
							lastEmail));
			...
			for (var agentPersona : agentPersonas) {
				...
				try {
					answer = MainService.getAssistant(comboModel.getSelectedItem().toString(), userPersona.get(0),
							userPersona.get(1), userPersona.get(2), agentPersona.get(0), agentPersona.get(1),
							agentPersona.get(2), new Tools()).chat(mail);
				} catch (Exception e) {
					e.printStackTrace();
					JOptionPane.showMessageDialog(frmAIMailExchangeSimulator,
							"There was an error fetching completions.", "Error", JOptionPane.ERROR_MESSAGE, null);
					return;
				}
	...
	private List<String> getPersonaFromName(String name) {
		for (int i = 0; i < personas.rowCount(); i++) {
			if (personas.get(i, 0).equals(name))
				return personas.getRow(i);
		}
		return null;
	}
	...
	private static String[] mailsColumns = new String[] { "date", "from", "to", "subject" };
		...
		btnGen.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!textAreaInput.getText().trim().isEmpty()) {
					interact(textAreaInput.getText(), textSubject.getText(), lastEmail(), getUserPersona(),
							getSelectedPersona());
				}
			}
		});
		...
		groupLayout.setHorizontalGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(groupLayout
				...
				.addGroup(groupLayout.createParallelGroup(Alignment.TRAILING).addGroup(groupLayout
						...
						.addGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
								.addComponent(lblNewLabel, Alignment.LEADING, GroupLayout.DEFAULT_SIZE, 1010,
										Short.MAX_VALUE)
								.addGroup(Alignment.LEADING, groupLayout.createSequentialGroup()
										.addGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
												.addGroup(groupLayout.createSequentialGroup()
														.addComponent(lblNewLabel_6, GroupLayout.PREFERRED_SIZE, 70,
																GroupLayout.PREFERRED_SIZE)
														.addPreferredGap(ComponentPlacement.RELATED)
														.addComponent(textSubject, GroupLayout.PREFERRED_SIZE, 701,
																GroupLayout.PREFERRED_SIZE)
														.addPreferredGap(ComponentPlacement.UNRELATED)
														.addComponent(btnClear, GroupLayout.PREFERRED_SIZE, 92,
																GroupLayout.PREFERRED_SIZE)
														.addGap(115))
												.addGroup(groupLayout.createSequentialGroup()
														.addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
																...
																.addComponent(lblNewLabel_8, GroupLayout.PREFERRED_SIZE,
																		98, GroupLayout.PREFERRED_SIZE)
																.addComponent(lblNewLabel_7, GroupLayout.PREFERRED_SIZE,
														...
														.addGroup(groupLayout
																...
																.addComponent(comboUserPersona, 0,
																		GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
														.addPreferredGap(ComponentPlacement.RELATED)
														.addComponent(lblNewLabel_2, GroupLayout.DEFAULT_SIZE,
																GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
														.addPreferredGap(ComponentPlacement.UNRELATED)
														.addComponent(scrollPane_4, GroupLayout.PREFERRED_SIZE, 341,
																GroupLayout.PREFERRED_SIZE)
														.addGap(289)))
										.addGap(18))
								.addGroup(Alignment.LEADING, groupLayout.createParallelGroup(Alignment.TRAILING, false)
										.addComponent(scrollPane_1, Alignment.LEADING, 0, 0, Short.MAX_VALUE)
										.addComponent(btnGen, Alignment.LEADING, GroupLayout.DEFAULT_SIZE,
												GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
										.addGroup(Alignment.LEADING, groupLayout.createSequentialGroup()
												.addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
														.addComponent(btnClearFiles).addComponent(btnAttachFiles))
		...
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(groupLayout
				...
				.addGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(groupLayout
						...
						.addGroup(groupLayout.createParallelGroup(Alignment.BASELINE).addComponent(lblNewLabel_8)
								.addComponent(comboEndpoints, GroupLayout.PREFERRED_SIZE, 22,
										GroupLayout.PREFERRED_SIZE)
								.addComponent(lblNewLabel_2, GroupLayout.PREFERRED_SIZE, 18,
										GroupLayout.PREFERRED_SIZE))
						.addGap(6)
						...
						.addGroup(groupLayout.createParallelGroup(Alignment.BASELINE).addComponent(lblNewLabel_1)
								.addComponent(comboUserPersona, GroupLayout.PREFERRED_SIZE, 25,
										GroupLayout.PREFERRED_SIZE)))
				...
				.addPreferredGap(ComponentPlacement.RELATED).addComponent(btnGen).addGap(18)
				.addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
						.addGroup(groupLayout.createSequentialGroup().addGap(25).addComponent(btnClearFiles))
		...
		tableMails.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == KeyEvent.VK_DELETE || e.getKeyCode() == KeyEvent.VK_BACK_SPACE) {
					...
					if (im >= 0)
						((DefaultTableModel) tableMails.getModel()).removeRow(im);
	...
	private int findSelectedMailIndexInMatrix(List<String> v) {
		for (int i = 0; i < mails.rowCount(); i++) {
			if (mails.get(i, 0).equals(v.get(0)) && mails.get(i, 1).equals(v.get(1)) && mails.get(i, 2).equals(v.get(2))
	...
	private int findSelectedMailIndexInModel(List<String> v) {
		for (int i = 0; i < tableMails.getModel().getRowCount(); i++) {
			if (tableMails.getModel().getValueAt(i, 0).equals(v.get(0))
	...
	/*
	 * private static Object buildToolFromClass(Class<?> cl) { UnitSourceGenerator
	 * unitSG = UnitSourceGenerator.create("Tools");
	 * 
	 * var gen =
	 * ClassSourceGenerator.create(TypeDeclarationSourceGenerator.create(cl.
	 * getSimpleName() + "Tool")) .addModifier(Modifier.PUBLIC);
	 * 
	 * for (Method method : cl.getMethods()) { if
	 * (!Modifier.isStatic(method.getModifiers())) continue;
	 * 
	 * var gen2 = FunctionSourceGenerator.create(method.getName())
	 * .addAnnotation(AnnotationSourceGenerator.create(dev.langchain4j.agent.tool.
	 * Tool.class)) .setReturnType(method.getReturnType());
	 * gen2.addModifier(Modifier.PUBLIC); String body = "return " +
	 * cl.getCanonicalName() + "." + method.getName() + "("; List<String> params =
	 * new ArrayList<String>(); for (Parameter p : method.getParameters()) { try {
	 ...
	 * 
	 * params.add(p.getName()); } body += Joiner.on(", ").join(params) + ");";
	 * gen2.addBodyCode(body); gen.addMethod(gen2); }
	 * 
	 * ComponentSupplier componentSupplier = ComponentContainer.getInstance();
	 * 
	 * ClassFactory classFactory = componentSupplier.getClassFactory();
	 * unitSG.addClass(gen); try { var classRetriever =
	 * classFactory.loadOrBuildAndDefine(unitSG);
	 * 
	 * Class<?> toolClass = classRetriever.get("Tools." + cl.getSimpleName() +
	 * "Tool"); return toolClass.getDeclaredConstructor().newInstance();
	 * 
