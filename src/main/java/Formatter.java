...
public class Formatter {
	...
	for (int i = 0; i < lines.size(); i++) {
		...
		for (String d : parentheseses) {
			...
			if ((1+last-first) > lineLength) {
				...
				for (int j = 0; j < potentialDelimiters.length; j++) {
				    if (potentialDelimiters[j].equals(firstchar))
					postblock = "";
				}
			    }
			    if(newLines.get(newLines.size()-1).length() + 1 + block.length() + postblock.length() + trailing.length() < lineLength) {
				newLines.set(newLines.size()-1, newLines.get(newLines.size()-1) + " " + block  + postblock + trailing);
			    } else if(newLines.get(newLines.size()-1).length() + 1 + block.length() < lineLength) {
				newLines.set(newLines.size()-1,newLines.get(newLines.size()-1) + " " + block);
				newLines.add(trailing);
			    } else {
				if(block.length() + postblock.length() + trailing.length() < lineLength) {
	...
	if (search >= 0) {
	    ...
	    lines.remove(i);
	    while (s.length() > 0) {
		boolean isLast = s.length() <= lineLength-indentation.length();
		lines.add(i + (x++), indentation + s.substring(0, Math.min(lineLength-indentation.length(), s.length())));
		if (!isLast) {
		    s = s.substring(lineLength-indentation.length());
		} else {
		    s = "";
		}
	    }
